...
async function fetchRemoteContent(
  ...
  if (document.destConnector.type === "GHOST") {
    ...
    return {
      ...
      updatedAt: post.posts?.[0]?.updated_at
        ? new Date(post.posts[0].updated_at)
        : new Date(),
    };
  }
...
async function fetchSourceContent(
  ...
  const config =
    (document.sourceConnector.config as Record<string, string>) || {};
  ...
  if (document.sourceConnector.type === "GOOGLE_DOCS") {
    ...
    return {
      ...
      title: docData.title,
      updatedAt: docData.modifiedTime
        ? new Date(docData.modifiedTime)
        : new Date(),
    };
  }
