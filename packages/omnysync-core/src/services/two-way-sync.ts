...
async function fetchRemoteContent(documentId: string): Promise<RemoteContent | null> {
  ...
  const rawCredentials = decrypt(document.destConnector.config?.credentials?.toString() || document.destConnector.credentials || "")
...
export async function detectConflicts(documentId: string): Promise<ConflictInfo> {
  ...
  const hasSourceChanged = document.sourceUpdatedAt && 
    sourceContent && 
    sourceContent.updatedAt > document.sourceUpdatedAt
...
export async function syncFromSource(documentId: string): Promise<TwoWaySyncResult> {
  ...
  if (!document || !document.sourceConnectorId || !document.destConnectorId) {
    return { success: false, direction: "none", changesDetected: false, message: "Document not found" }
  }

  // Récupérer le contenu source actuel
  const sourceContent = await fetchSourceContent(documentId)
  if (!sourceContent) {
    return { success: false, direction: "none", changesDetected: false, message: "Source content not available" }
  }
...
export async function syncFromDest(documentId: string): Promise<TwoWaySyncResult> {
  ...
  if (!destContent) {
    return { success: false, direction: "none", changesDetected: false, message: "Destination content not available" }
  }
...
export async function resolveConflict(
  ...
  if (!conflict.hasConflict) {
    return { success: true, direction: "none", changesDetected: false, message: "No conflict to resolve" }
  }
  ...
  switch (strategy) {
    ...
    case "keep-both":
      ...
      const destContent = await fetchRemoteContent(documentId)
      if (!destContent) {
        return { success: false, direction: "none", changesDetected: false, message: "Could not fetch destination content" }
      }
    ...
    default:
      return { success: false, direction: "none", changesDetected: false, message: "Unknown strategy" }
  }
}
...
export async function checkAndAutoSync(documentId: string): Promise<TwoWaySyncResult> {
  ...
  if (!conflict.hasConflict) {
    return { success: true, direction: "none", changesDetected: false, message: "No changes detected" }
  }

  // Par défaut, on sync depuis la source (comportement standard)
  await auditSync.changesDetected((await prisma.document.findUnique({ where: { id: documentId } }))!.organizationId, documentId)
