...
export function createShopifyClient(shopDomain: string, accessToken: string) {
  ...
  return {
    ...
    async createArticle(blogId: string, article: ShopifyArticle): Promise<{ article: { id: string } }> {
    ...
    async uploadImage(image: { attachment: string; filename: string }): Promise<{ asset: { src: string } }> {
