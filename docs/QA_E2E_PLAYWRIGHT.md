# QA end-to-end avec Playwright (Omnysync)
...
Start-Process -FilePath cmd -ArgumentList "/c","pnpm dev (depuis omnysync-web)"
-RedirectStandardOutput "$env:TEMP\qa\Omnysync.dev.txt" 
  -RedirectStandardError "$env:TEMP\qa\Omnysync.err.txt" -WindowStyle Hidden
...
Aucun bypass d auth dev trouve dans omnysync-web. Comme / renvoie 500, meme les pages publiques ne sont pas servies tant que le probleme de middleware/env n est pas resolu.
...
`omnysync-web/scripts/qa\        # (Ã  crÃ©er si campagne complÃ¨te)
