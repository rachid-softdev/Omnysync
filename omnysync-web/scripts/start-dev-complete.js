...
console.log('🚀 Starting Omnysync Complete Development Environment...\n');
...
function detectDatabaseProvider() {
  ...
  if (fs.existsSync(prismaSchemaPath)) {
    ...
    const match = schemaContent.match(/provider\s*=\s*"(\w+)"/);
...
function checkPostgres() {
  ...
  if (provider === 'postgresql') {
    console.log('✅ PostgreSQL detected (via Prisma schema)');
    return true;
  }
...
function setupDatabase() {
  const provider = detectDatabaseProvider();
  try {
    console.log(`🗄️  Setting up ${provider} database...`);
    ...
    console.error(`❌ ${provider} database setup failed:`, error.message);
    console.log('\n💡 Make sure your database is running:');
    console.log('   • PostgreSQL server installed and started (or cloud-hosted like Neon)');
    console.log('   • Database exists');
    console.log('   • DATABASE_URL correct in .env.local');
...
function setupStripeProducts() {
  try {
    ...
    execSync('npx tsx scripts/create-stripe-products.ts', { stdio: 'pipe' });
...
async function main() {
  try {
    ...
    if (!checkPostgres() || !setupDatabase()) process.exit(1);
    console.log('');
    ...
    console.log('   • PostgreSQL        → Neon (cloud) or localhost:5432');
