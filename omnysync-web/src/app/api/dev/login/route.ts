...
export async function POST() {
  ...
  await prisma.user.upsert({
    ...
    update: { password: passwordHash, role: 'ADMIN', disabledAt: null, deletedAt: null },
    create: {
      email: DEV_EMAIL,
      name: 'Dev User',
  ...
  if (result?.error) {
    return NextResponse.json({ error: 'Dev sign-in failed', detail: result.error }, { status: 401 })
  }
